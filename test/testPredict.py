...
save_dir = os.path.join(current_dir, 'weedSpec1/predTest/')
...
for img_num in image_list:
	...
	numpy.savetxt('%s%s(label).csv'%(save_dir,img_num), label, delimiter=",")
	...
	res_2 = result[:,:,1]
	print(res_1.shape)
	numpy.savetxt('%s%s(0).csv'%(save_dir,img_num), res_1, delimiter=",")
	numpy.savetxt('%s%s(1).csv'%(save_dir,img_num), res_2, delimiter=",")
